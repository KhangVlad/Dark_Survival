...
public class UIVirtualJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public bool allowDraggingOutside = true;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        {
            Vector2 normalized = ApplySizeDelta(position);
            Vector2 clamped = ClampValuesToMagnitude(normalized);
            Vector2 output = ApplyInversionFilter(clamped);

            OutputPointerEventValue(output * magnitudeMultiplier);
