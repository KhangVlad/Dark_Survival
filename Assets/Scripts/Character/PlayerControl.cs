...
public class PlayerControl : MonoBehaviour
{
    ...
    void HandleInput()
    {
        if (Input.GetKeyDown(KeyCode.W)) moveInput.y = 1f; // On press set move up
        if (Input.GetKeyUp(KeyCode.W)) moveInput.y = 0f; // On release reset vertical movement if no other key pressed

        if (Input.GetKeyDown(KeyCode.S)) moveInput.y = -1f;
        if (Input.GetKeyUp(KeyCode.S)) moveInput.y = 0f;

        if (Input.GetKeyDown(KeyCode.D)) moveInput.x = 1f;
        if (Input.GetKeyUp(KeyCode.D)) moveInput.x = 0f;

        if (Input.GetKeyDown(KeyCode.A)) moveInput.x = -1f;
        if (Input.GetKeyUp(KeyCode.A)) moveInput.x = 0f;

        moveInput = moveInput.normalized;
        thirdPersonController._input.MoveInput(moveInput);
